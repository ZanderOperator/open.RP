...
//=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~ (vars, iters, rBits)
new 
	PlayerText:srobTD[MAX_PLAYERS][17],
...
StorageRob(playerid, storage_id) {
	...
	if (storage_id != -1 && HouseStorage[storage_id][storageExists])
	{
		new string[256];
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid) {
	    case DIALOG_ROB_STORAGE: {
			if(response) {
				new storage_id = Storage_RackNear(playerid),
