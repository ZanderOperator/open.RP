...
//=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~ (vars, iters, rBits)
new p_EditRack[MAX_PLAYERS]	= INVALID_PLAYER_ID,
...
{
	new query[ 256 ];	
	format(query, 256, "UPDATE `house_storage` SET `storageHouse` = '%d', `storageX` = '%.4f', `storageY` = '%.4f', `storageZ` = '%.4f', `storageA` = '%.4f', `storageInterior` = '%d', `storageWorld` = '%d' WHERE `storageID` = '%d'",
...
{
	new query[ 256 ];	
	format(query, 256, "UPDATE `house_storage` SET `gunID%d` = '%d', `ammoID%d` = '%d' WHERE `storageID` = '%d'",
		wepid,
...
Storage_Refresh(storageid)
{
    if (storageid != -1 && HouseStorage[storageid][storageExists])
	{
		...
		for (new i = 0; i < 4; i ++) if (IsValidDynamicObject(HouseStorage[storageid][storageObjects][i])) {
...
Storage_RackDelete(storageid)
{
    if (Iter_Contains(HStorage_Iter, storageid))
	{
		if(HouseStorage[storageid][storageExists])
		{
			...
			format(string, sizeof(string), "DELETE FROM `house_storage` WHERE `storageID` = '%d'", HouseStorage[storageid][storageID]);
...
//=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~ (return)
forward OnRackCreated(storageid, playerid);
...
CMD:check_gunrack(playerid, params[]) 
{
	...
	if (storageid == -1) 
		return SendErrorMessage(playerid, "Morate biti u blizini Stalka.");
