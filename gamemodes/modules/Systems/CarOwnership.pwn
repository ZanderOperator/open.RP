...
new
	...
	Iterator:COWeapons		[MAX_VEHICLES]<MAX_WEAPON_SLOTS>,
...
stock PutPlayerWeaponInTrunk(playerid, vehicleid, weaponid)
{
	new Float:x, Float:y, Float:z, Float: rz,
		model = trunk_WeaponModels(weaponid),
	...
	if(!IsANoTrunkVehicle(vmodel) && !IsABike(model) && !IsAMotorBike(model))
	{
		...
		EditingTrunkWeaponModel[playerid] = trunk_WeaponModels(weaponid);
...
stock RespawnTrunkObjects(vehicleid)
{
	foreach(new wslot: COWObjects[vehicleid])
	{
		if(IsValidObject(VehicleInfo[vehicleid][vWeaponObjectID][wslot]))
		{
			VehicleInfo[vehicleid][vWeaponObjectID][wslot] = CreateObject(trunk_WeaponModels(VehicleInfo[vehicleid][vWeaponId][wslot]), VehicleInfo[vehicleid][vOffsetx], VehicleInfo[vehicleid][vOffsety], VehicleInfo[vehicleid][vOffsetz], VehicleInfo[vehicleid][vOffsetxR], VehicleInfo[vehicleid][vOffsetyR], VehicleInfo[vehicleid][vOffsetzR], 50.0);
...
stock static IsHaveNoNumberPlate(model)
{
    switch(model)
	{
		...
		{	
			...
			foreach(new wslot: COWeapons[vehicleid])
			{
				if(VehicleInfo[vehicleid][vWeaponSQLID][wslot] == tmpSQL)
				{
					...
					VehicleInfo[vehicleid][vWeaponObjectID][wslot] = CreateObject(trunk_WeaponModels(VehicleInfo[vehicleid][vWeaponId][wslot]), Pos[0], Pos[1], Pos[2], 0, 0, 0, 0);	
					AttachObjectToVehicle(VehicleInfo[vehicleid][vWeaponObjectID][wslot], vehicleid, VehicleInfo[vehicleid][vOffsetx][wslot], VehicleInfo[vehicleid][vOffsety][wslot], VehicleInfo[vehicleid][vOffsetz][wslot], VehicleInfo[vehicleid][vOffsetxR][wslot], VehicleInfo[vehicleid][vOffsetyR][wslot], VehicleInfo[vehicleid][vOffsetzR][wslot]);
...
/*
	d8888b. db    db db    db      db    db d88888b db   db
	88  `8D 88    88 `8b  d8'      88    88 88'     88   88
	88oooY' 88    88  `8bd8'       Y8    8P 88ooooo 88ooo88
	88~~~b. 88    88    88         `8b  d8' 88~~~~~ 88~~~88
...
stock ParkVehicleInfo(vehicleid)
{
	...
	Iter_Remove(COVehicles, vehicleid);
	return 1;
}
...
stock ResetVehicleAlarm(vehicleid)
{
	...
	Bit1_Set(gr_VehicleBlinking, vehicleid, false);
...
stock static StartVehicleAlarm(vehicleid)
{
	if(vehicleid == -1) 			return 0;
	if(Bit1_Get(gr_VehicleAlarmStarted, vehicleid)) return 0;
	...
	Bit1_Set(gr_VehicleAlarmStarted, vehicleid, true);
	return 1;
}
...
*/
///////////////////////////////////////////////////////////////////
Function: VehicleTowTimer(vehicleid, playerid)
{
...
public VehicleLightsBlink(vehicleid)
{
	...
	if(!VehicleLightsBlinker[vehicleid]) {
		...
		GetVehicleDamageStatus(vehicleid, param[0], param[1], param[2], param[3]);
		if(!Bit1_Get(gr_VehicleBlinking, vehicleid))
		{
			param[2] = encode_lights(1, 1, 0, 0);
			Bit1_Set(gr_VehicleBlinking, vehicleid, true);
		}
		else
		{
...
Function: VehicleGlobalTimer()
{
	if(gettimestamp() >= VehicleTimer && gettimestamp() >= GlobalVehicleStamp)
	{
		...
		{
			if(VehicleInfo[vehicleid][vUsage] == VEHICLE_USAGE_PRIVATE)		//CO je!
			{
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		...
		case DIALOG_VEH_JUNK_SELL:
		{
			...
			if(!Iter_Contains(COVehicles, vehicleid)) return SendMessage(playerid, MESSAGE_TYPE_ERROR, "Vase vozilo nije CO!");
...
CMD:car(playerid, params[])
{
	...
	else if(!strcmp(pick, "tow", true))
	{
		...
		foreach (new i : Player)
		{
			if(IsPlayerInVehicle(i, vehicleid)) return SendMessage(playerid, MESSAGE_TYPE_ERROR, "Netko vec koristi vase vozilo.");
			break;
		}
		if(PlayerInfo[playerid][pDonateRank] < 2)
		{
			...
			PlayerToOrgMoneyTAX(playerid, FACTION_TYPE_LAW, 1000); // Novac ide PDu uz porez koji ide drzavi
		}
		// Timer
	...
	else if(!strcmp(pick, "register", true))
	{
		...
		PlayerToOrgMoneyTAX(playerid, FACTION_TYPE_LAW, 1000); // Novac ide u PD, dok oporezivi dio ide drZavi
