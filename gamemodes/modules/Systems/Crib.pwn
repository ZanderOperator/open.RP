...
CMD:enter(playerid, params[])
{
	...
	else if(rcomplex != INVALID_COMPLEX_ID)
	{
		...
		if(PlayerInfo[playerid][pComplexRoomKey] == rcomplex || Bit1_Get(a_AdminOnDuty, playerid) || !ComplexRoomInfo[rcomplex][cLock]) 
		{
			Bit1_Set(gr_PlayerEntering, playerid, true);
	...
	else if(IsPlayerInDynamicCP(playerid, PlayerHouseCP[playerid])) 
	{
		...
		if(HouseInfo[house][h3dViwo] == GetPlayerVirtualWorld(playerid)) 
		{
			...
			if(PlayerInfo[playerid][pHouseKey] == house || Bit1_Get(a_AdminOnDuty, playerid) || !HouseInfo[house][hLock] || PlayerInfo[playerid][pRentKey] == house) 
			{
				...
				if(HousePlayingMusic[house]) {
					StopAudioStreamForPlayer(playerid);
					PlayAudioStreamForPlayer(playerid, HouseMusicURL[house]);
				}
