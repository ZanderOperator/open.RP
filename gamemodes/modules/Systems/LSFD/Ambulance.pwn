...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	{	
		if( 2 <= GetPlayerVehicleSeat(playerid) <= 4 ) {
			SetPlayerPosEx(playerid, 1557.8597, 1514.3241, -15.3713, GetPlayerVehicleID(playerid), 1, true);
			SetCameraBehindPlayer(playerid);
			Bit16_Set( gr_PlayerAmbulanceId, playerid, GetPlayerVehicleID(playerid) );
		}
	}
	return 1;
}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	{
		if( Bit16_Get( gr_PlayerAmbulanceId, playerid ) > 0 && Bit16_Get( gr_PlayerAmbulanceId, playerid ) != INVALID_VEHICLE_ID ) {
			new 
				Float:X, Float:Y, Float:Z;
			GetVehiclePos( Bit16_Get( gr_PlayerAmbulanceId, playerid ), X, Y, Z );
