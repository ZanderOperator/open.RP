	...
	if(pEditingATM[playerid] != -1)
	{
	    if(objectid == ATMInfo[pEditingATM[playerid]][atm_objid])
	    {
			...
			else if(response == EDIT_RESPONSE_CANCEL)
			{
				...
				va_SendClientMessage(playerid, COLOR_RED, "[INFO]: Odustao si od ure�ivanja ATM-a %d!", pEditingATM[playerid]);
				pEditingATM[playerid] = -1;
...
CMD:atm(playerid, params[])
{
	...
	if(IsPlayerInAnyVehicle(playerid))
	    return SendClientMessage(playerid, COLOR_RED, "[GRESKA]: Ne mo�e� koristiti ovu komandu dok si u vozilu!");
    ...
    {
		format(string, sizeof(string), "ATM: Trenutno stanje na va�em ra�unu je: %d$!", PlayerInfo[playerid][pBank]);
...
CMD:aatm(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] != 1338)
	    return SendClientMessage(playerid, COLOR_RED, "[GRESKA]: Ne mo�ete koristiti ovu komandu!");
	...
	switch(option)
	{
	    case 1:
	    {
			...
			if((ATM_id = ATM_Create(pi_x, pi_y, pi_z, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid))) != -1)
			    ...
			    SendClientMessage(playerid, COLOR_RED, "[GRE�KA]: Nema slobodnih ATM slotova!");
	    ...
	    case 2:
	    {
			...
			if(id < 0 || id > 50)
			    return SendClientMessage(playerid, COLOR_RED, "[GRE�KA]: Nepravilan ID ATM-a!");
			    
			if(!Iter_Contains(Atm, id))
			    return SendClientMessage(playerid, COLOR_RED, "[GRE�KA]: Taj ATM nije kreiran!");
			...
			if(!IsPlayerInRangeOfPoint(playerid, 10.0, ATMInfo[id][atm_x], ATMInfo[id][atm_y], ATMInfo[id][atm_z]) && GetPlayerInterior(playerid) != ATMInfo[id][atm_interior] && GetPlayerVirtualWorld(playerid) != ATMInfo[id][atm_virtualworld])
			    return SendClientMessage(playerid, COLOR_RED, "[GRE�KA]: Niste blizu tog ATM-a!");
	    ...
	    case 3:
	    {
            ...
            if(ATM_Delete(id))
                return va_SendClientMessage(playerid, COLOR_RED, "[INFO]: Uspje�no si izbrisao ATM %d sa servera!", id);
			else
			    return SendClientMessage(playerid, COLOR_RED, "[GRE�KA]: Taj ATM nije kreiran!");
	    }
	    default:
