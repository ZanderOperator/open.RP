...
stock static InitPlayerPocket(playerid, targetid)
{
	...
	if( AC_GetPlayerMoney(targetid) >  50 ) {//( ( GetPlayerSkillLevel(playerid, 5) + 1 ) * 10 ) ) {
...
{
	if( PlayerInfo[ playerid ][ pJob ] != ROBBER_ID )
		return SendMessage(playerid, MESSAGE_TYPE_ERROR, "Niste lopov!");
	if( RobbingInfo[ playerid ][ rbSlotsGoods ][ 0 ] == 0 && RobbingInfo[ playerid ][ rbSlotsGoods ][ 1 ] == 0 && RobbingInfo[ playerid ][ rbSlotsGoods ][ 2 ] == 0 && RobbingInfo[ playerid ][ rbSlotsGoods ][ 3 ] == 0 && RobbingInfo[ playerid ][ rbSlotsGoods ][ 4 ] == 0 ) 
		return SendMessage(playerid, MESSAGE_TYPE_ERROR, "Vas inventory je prazan!");
...
CMD:stealmoney(playerid, params[])
{
	...
	if( HouseInfo[ house ][ hMoneySafe ] )
	{
		...
		else SendMessage(playerid, MESSAGE_TYPE_ERROR, "Niste uspjeli uzeti novce iz kase!");
	} else {
		rand = random(20) + 1;
		...
		else SendMessage(playerid, MESSAGE_TYPE_ERROR, "Niste uspjeli uzeti novce!");
	}
	return 1;
}
