...
CMD:id(playerid, params[])
{
	...
	else
	{
		...
		if(!count)
			SendMessage(playerid, MESSAGE_TYPE_ERROR, "Nijedan korisnik nije pronaï¿½en sa navedenim imenom!");
	}
	return 1;
}
CMD:changeseat(playerid, params[])
{
	...
	if(seatid > GetVehicleSeats(GetVehicleModel(vehicleid)) - 1) return SendMessage(playerid, MESSAGE_TYPE_ERROR, "Vozilo nema toliko sjedeÄ‡ih mjesta!");
...
CMD:changename(playerid, params[])
{
	...
	if( years > 100 || years < 16)
	    return SendMessage(playerid, MESSAGE_TYPE_ERROR, "Godine nemogu biti veï¿½e od 100 ni manje od 16!");
...
CMD:accent(playerid, params[])
{
	...
	if(strlen(params) > 18 || strlen(params) < 3)
	    return SendClientMessage(playerid, COLOR_RED, "Naglasak ne moZe biti veï¿½i od 18 karaktera ni manji od 3 karaktera.");
...
CMD:usecigarette(playerid, params[])
{
	...
	if( !PlayerInfo[playerid][pLighter] ) return SendClientMessage(playerid, COLOR_RED, "[GRESKA]: NemaÅ¡ upaljac!");
