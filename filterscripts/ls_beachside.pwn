...
#define COLOR_MESSAGE_YELLOW        0xD8D8D8AA
...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Pritisni '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' za koristenje lifta.", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, z - 0.4, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            GameTextForPlayer(playerid, "~r~Lift je vec na katu.", 3500, 4);
		else if(DidPlayerRequestElevator(playerid))
		    GameTextForPlayer(playerid, "~r~Lift je vec pozvan.", 3500, 4);
		else
		...
		else
		{
		    ...
		    if(pos[1] < (Y_ELEVATOR_POS - 1.81) && pos[1] > (Y_ELEVATOR_POS - 3.8) && pos[0] > (X_ELEVATOR_POS + 1.21) && pos[0] < (X_ELEVATOR_POS + 3.8))
		    {
				...
				if(i <= 12)
				{
					...
					SendClientMessage(playerid, 0xC2A2DAAA, strTempString);
...
stock Elevator_Initialize()
{
	...
	Label_Elevator = Create3DTextLabel("{CCCCCC}Pritisni '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' za koristenje lifta.", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0, 0, 1);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Pritisni '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' da pozoves lift.", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "LS BeachSide Lift", string, "Pozovi", "Odustani");
