...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Pritisni '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' za koristenje lifta.", 0xCCCCCCAA, 1784.9822, -1302.0426, z - 0.9, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
		    ...
		    GameTextForPlayer(playerid, "~r~Vec si pozvao lift.", 3500, 4);
		else
		...
		else    // Is he in a floor button?
		{
		    if(pos[1] > -1301.4 && pos[1] < -1299.1447 && pos[0] < 1785.6147 && pos[0] > 1781.9902)
		    {
				...
				if(i <= 19)
				{
					...
					GameTextForPlayer(playerid, "~r~Lift pozvan", 3500, 4);
				}
		    }
		}
	}
...
stock Elevator_Initialize()
{
	...
	Label_Elevator          = Create3DTextLabel("{CCCCCC}Pritisni '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' za koristenje lifta.", 0xCCCCCCAA, 1784.9822, -1302.0426, 13.6491, 4.0, 0, 1);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "Lift", string, "Pozovi", "Odustani");
